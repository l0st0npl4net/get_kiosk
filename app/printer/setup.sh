...
sudo cp app/printer/cupsd.conf /etc/cups/cupsd.conf

if [[ -f /etc/sst-iiko/print_settings.ini ]] || [[ -d /etc/sst-iiko/templates ]]; then 
      echo "Already exists!"
else 
      sudo cp app/printer/print_settings.ini /etc/sst-iiko/print_settings.ini
      sudo mkdir /etc/sst-iiko/templates
      sudo cp app/printer/receipt.atdf /etc/sst-iiko/templates
fi
...
cd app/printer/driver/"$PH"/KPOS_Printer/filter && sudo chmod +x ./install.sh && sudo ./install.sh
